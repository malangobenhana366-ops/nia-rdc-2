...
// ================= INTEGRATION COMPLÈTE ET EXACTE DES CONTENUS LÉGAUX REMPLACÉS =================
...
function basculerAffichageAuthentification(versInscription) {
  ...
  document.getElementById("form-register-block").style.display = versInscription ? "block" : "none";
  document.getElementById("form-login-block").style.display = versInscription ? "none" : "block";
}
...
// LOGIQUE DES COUCHES DE ZOOM PHOTOS FLUIDES (LIGHTBOX)
function agrandirPhoto(srcUrl) {
...
function rendreFluxHtml(liste) {
  ...
  liste.forEach(a => {
    let imagesMarkup = (a.images && a.images.length > 0) ? `<div class="gallery">${a.images.map(imgObj => `<img src="${imgObj.url}" onclick="agrandirPhoto('${imgObj.url}')">`).join("")}</div>` : "";
