...
function abrirSecuriseModal(id) {
...
async function ouvrirMessagerieDirecteInstantane(annonceId, titreAnnonce) {
  ...
  if(res.ok) alert("Message privé envoyé au propriétaire ! Retrouvez le fil dans votre Espace Privé.");
  else alert("Erreur lors de l'envoi du message privé.");
}
...
async function chargerConversationsPrivees() {
  ...
  if(data.length === 0) { box.innerHTML = "Aucun message privé reçu ou envoyé pour le moment."; return; }

  box.innerHTML = data.map(c => `
    <div style="background:white; padding:8px; border-radius:6px; border:1px solid var(--border); font-size:0.85rem;">
      <div style="font-weight:bold; color:var(--primary);">Sujet Immo : ${c.annonce_titre}</div>
...
// ROUTAGE ET FACETTES PERMANENTES DE L'ESPACE PRIVÉ AVEC MODIFICATION/BOOST/SUPPRESSION DIRECTE
function basculerOngletProfil(mode) {
  ...
  if(userList.length === 0) { listDiv.innerHTML = "<p style='color:gray; text-align:center;'>Aucun bien identifié dans cette catégorie.</p>"; return; }
  ...
  listDiv.innerHTML = userList.map(a => `
    <div style="background:#f8fafc; padding:12px; border-radius:8px; border:1px solid var(--border); margin-bottom:8px;">
      <div style="display:flex; justify-content:space-between; align-items:center; width:100%;">
        <div style="font-weight:bold; font-size:0.9rem;">${a.titre} - ${a.prix} ${a.devise} / ${a.periode} [${a.statut === 'occupe' ? '🔴 Occupé' : '🟢 Disponible'}]</div>
...
async function sauvegarderChangementsAnnonce() {
  ...
  await fetch(`${API}/annonces/${id}`, {
    ...
    body: JSON.stringify({
      titre: document.getElementById("edit-edit-titre") ? document.getElementById("edit-edit-titre").value : document.getElementById("edit-titre").value,
...
// SYSTEME DE NOTIFICATIONS ET DE DE JUSTIFICATIONS DE L'ESPACE PRIVÉ
async function chargerAlertesAdmin() {
  ...
  if(data.length === 0) { c.innerHTML = "Aucun message administratif ou demande d'explication."; return; }
  c.innerHTML = data.map(m => `
    <div style="border-bottom:1px solid #cbd5e1; padding-bottom:6px; margin-bottom:6px; color:var(--text); background:rgba(255,255,255,0.6); padding:6px; border-radius:4px;">
      <b>🚨 Message Admin :</b> ${m.message}
      ${m.reponse_utilisateur ? `<div style="color:var(--success); font-weight:bold; padding-left:10px; font-size:0.8rem; margin-top:4px;">Justification transmise : "${m.reponse_utilisateur}"</div>` : `
        <div style="margin-top:6px; display:flex; gap:4px;"><input id="alert-rep-${m.id}" placeholder="Tapez votre justification ici..." style="font-size:0.8rem; flex:1; padding:4px; border:1px solid var(--border); border-radius:4px;"><button class="btn-auth" onclick="soumettreReponseJustificative(${m.id})" style="font-size:0.75rem; padding:4px 8px; background:var(--primary);">Envoyer</button></div>`}
    </div>`).join("");
}
...
// VUES DE CENTRALISATION ET LOGIQUE DE RECHERCHE PAR ID SÉCURISÉ (ANTI-CRASH ESPACES/CARACTÈRES)
async function definirVueAdmin(mode) {
  ...
  if(mode === "flux") {
    box.innerHTML = toutesLesAnnonces.map(a => `
      <div style="background:#1e293b; padding:8px; border-radius:4px; font-size:0.85rem; border-bottom:1px solid #334155;">
        <span><b>${a.titre}</b> (Id unique: ${a.id} | Tel: ${a.telephone})</span>
        <div style="display:flex; gap:4px; margin-top:4px;">
          <input id="adm-input-${a.id}" placeholder="Rédiger un avertissement ciblé..." style="flex:1; color:black; padding:4px; border-radius:4px; border:none;">
          <button onclick="envoyerMessageAvertissementCible('${a.id}', '${a.telephone}', 'normal')" style="background:var(--success); color:white; border:none; padding:4px 8px; cursor:pointer; border-radius:4px;">Notifier</button>
          <button onclick="supprimerAnnonceParAdmin(${a.id})" style="background:var(--danger); color:white; border:none; padding:4px 8px; cursor:pointer; border-radius:4px;">🗑️ Supprimer</button>
        </div>
      </div>`).join("");
  }
  else if(mode === "signaux") {
    const res = await fetch(`${API}/admin/reports`); const data = await res.json();
    if(data.length === 0) { box.innerHTML = "Aucun signalement actif recensé."; return; }
    box.innerHTML = data.map(r => `
      <div style="background:#1e293b; padding:8px; border-radius:4px; border-left:4px solid var(--danger); font-size:0.85rem; border-bottom:1px solid #334155;">
        <div style="color:#f87171; font-weight:bold;">🚨 MOTIF : "${r.raison}"</div>
        <span>Annonce: ${r.titre} | Id: ${r.id} | Tel: ${r.telephone}</span>
        <div style="display:flex; gap:4px; margin-top:4px;">
          <input id="adm-input-${r.id}" placeholder="Exiger une justification immédiate..." style="flex:1; color:black; padding:4px; border-radius:4px; border:none;">
          <button onclick="envoyerMessageAvertissementCible('${r.id}', '${r.telephone}', 'signale')" style="background:orange; color:white; border:none; padding:4px 8px; cursor:pointer; border-radius:4px;">Exiger Justif</button>
          <button onclick="supprimerAnnonceParAdmin(${r.id})" style="background:var(--danger); color:white; border:none; padding:4px 8px; cursor:pointer; border-radius:4px;">Bannir</button>
        </div>
      </div>`).join("");
  }
    ...
    if(data.length === 0) { box.innerHTML = "Aucune justification en attente dans cette catégorie."; return; }
    box.innerHTML = data.map(m => `
      <div style="background:#1e293b; padding:8px; border-radius:4px; font-size:0.85rem; margin-bottom:4px;">
        <div style="color:#94a3b8;"><b>Message d'origine Admin :</b> ${m.message}</div>
        <div style="color:#4ade80; margin-top:4px; font-weight:bold;"><b>↩️ Justification reçue de l'utilisateur :</b> "${m.reponse_utilisateur}"</div>
      </div>`).join("");
  }
}

async function envoyerMessageAvertissementCible(idUniqueAnnonce, telDestinataire, ctx) {
  const msg = document.getElementById(`adm-input-${idUniqueAnnonce}`).value.trim(); if(!msg) return;
  await fetch(`${API}/admin/message`, {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ target_tel: telDestinataire, message: msg, is_global: false, provenance_contexte: ctx })
  });
  alert("Message d'avertissement envoyé avec succès (ID ciblé sans risque de crash)."); 
  document.getElementById(`adm-input-${idUniqueAnnonce}`).value = "";
...
async function diffuserNotificationGlobale() {
  ...
  alert("Message flash général envoyé à tous les comptes."); document.getElementById("admin-global-txt").value = "";
}

async function supprimerAnnonceParAdmin(id) { if(confirm("Valider la suppression administrative de ce bien ?")) { await fetch(`${API}/annonces/${id}/delete`, { method: "DELETE" }); chargerFluxPrincipal(); setTimeout(() => definirVueAdmin(VUE_ADMIN_ACTIVE), 400); } }
